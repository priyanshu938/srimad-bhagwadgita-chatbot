...
function buildContext(matches: RankedMatch[]): string {
    return matches
        .map((match, index) => {
            const snippet = match.record.text.replace(/\s+/g, " ").trim().slice(0, 1200);
            return `[Chunk ${index + 1}] ${snippet}`;
        })
        .join("\n\n");
}

async function answerWithGroq(question: string, matches: RankedMatch[]): Promise<string> {
        ...
        messages: [
            {
                role: "system",
                content:
                    "You are a Bhagavad Gita assistant. Answer only from the provided context. If context is insufficient, say you do not know based on current context."
            },
            {
                role: "user",
...
export async function chat() {
    ...
    while (true) {
        ...
        // Retrieve best chunks and send them to Groq for grounded answer generation.
        const matches = topMatches(question, indexedRecords, idfMap, TOP_K);
        let answer: string;
        try {
            answer = await answerWithGroq(question, matches);
        } catch {
            // Fallback keeps chatbot usable if LLM call fails.
            answer = answerFromMatches(question, matches);
        }
